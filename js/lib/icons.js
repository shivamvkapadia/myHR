...
export function icon(name, cls = "") {
  return raw(
    `<svg class="i ${cls}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${P[name] || P.circle}</svg>`
  );
}

export function logo(size = 22) {
  return raw(
    `<svg class="logo-mark" width="${size}" height="${size}" viewBox="0 0 24 24" aria-hidden="true"><rect width="24" height="24" rx="6.5" fill="var(--accent)"/><path d="M7 8.2h10M7 12h5.5M7 15.8h7.5" stroke="var(--accent-ink)" stroke-width="2.1" stroke-linecap="round"/><circle cx="16.6" cy="12" r="1.35" fill="var(--accent-ink)"/></svg>`
  );
}
